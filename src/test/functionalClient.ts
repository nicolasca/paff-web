...
type Entry = { module: 'games' | 'decks' | 'players'; name: string; user: number; args: Record<string, unknown>; value?: unknown; error?: unknown; listeners: Set<() => void> }
...
export function createFunctionalTransport(harness = createGameHarness()) {
  ...
  function parse(reference: string): Pick<Entry, 'module' | 'name'> {
    ...
    if (module !== 'games' && module !== 'decks' && module !== 'players') throw new Error(`Unexpected API request: ${reference}`)
    return { module, name }
  }
  return {
    harness,
