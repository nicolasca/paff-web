...
import { canDeployUnit, cells, deploymentLimit } from '../../../shared/board'
...
export function SetupPhases({ game, busy, perform }: { game: Game; busy: boolean; perform: (action: () => Promise<unknown>) => Promise<void> }) {
  ...
  const allowed = myTurn && profile ? cells.filter((cell) => canDeployUnit(cell, me.seat, profile, setup, artilleryOnly, artilleryRemaining)) : []
