...
export function PreparationPhase({ game, busy, perform }: { game: Game; busy: boolean; perform: (action: () => Promise<unknown>) => Promise<void> }) {
  ...
  return <section aria-label="Choix des unités à déployer">
    ...
    <div className="deployment-summary">
      <dl><div><dt>Unités choisies</dt><dd>{me.preparationCount}</dd></div><div><dt>Unités en réserve</dt><dd>{me.drawPileCount}</dd></div><><div><dt>Points déployés</dt><dd>{budget.deployed} / 21</dd></div><div><dt>Points en réserve</dt><dd>{budget.reserve} / 12</dd></div></></dl>
