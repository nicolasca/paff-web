...
export function UnitProfileStats({ profile, compact = false }: { profile: UnitProfile; compact?: boolean }) {
  return <dl className={compact ? 'battle-card__stats' : 'unit-card__stats'} aria-label="Profil de l’unité">
    ...
    <div className="unit-stat--offense">
      <dt><abbr title={profile.offense.kind === 'ranged' ? 'Tir' : 'Corps à corps'}>Attaque</abbr></dt>
      <dd aria-label={`${profile.offense.kind === 'ranged' ? 'Valeur d’attaque au tir' : 'Valeur d’attaque au corps à corps'} : ${profile.offense.score ?? '—'}`}>
        {profile.offense.score ?? '—'}<span className="unit-stat__mode" aria-label={profile.offense.kind === 'ranged' ? 'Tir' : 'Corps à corps'} title={profile.offense.kind === 'ranged' ? 'Tir' : 'Corps à corps'}>{profile.offense.kind === 'ranged' ? 'T' : 'C'}</span>
      </dd>
    </div>
