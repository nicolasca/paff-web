...
describe('functional two-player journey with real game handlers', () => {
  it('selects a private subset before initiative, restores it on reload, and places exactly those units', async () => {
    ...
    await stage('Prenez position')
    expect(p(1).getByRole('button', { name: 'Terminer mon déploiement' })).toBeDisabled()
    expect(p(1).getByRole('button', { name: 'E5 · Archers · Joueur 1' })).toBeVisible()
    ...
    for (const user of [1, 2]) {
      ...
      expect(p(user).getByRole('button', { name: 'E5 · Archers · Joueur 1' })).toBeVisible()
