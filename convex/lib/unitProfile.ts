...
export const unitProfileValidator = v.object({
  ...
  regiment: v.number(),
  dice: v.number(),
  offense: v.object({ kind: v.union(v.literal('melee'), v.literal('ranged'), v.literal('none')), score: v.union(v.number(), v.null()) }),
  defenseMelee: v.number(),
  defenseRanged: v.number(),
  defenseRangedFormat: v.optional(v.literal('threshold')),
  ability: v.optional(v.object({ id: v.optional(v.string()), name: v.string(), description: v.string() })),
