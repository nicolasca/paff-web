...
describe('unit selection before initiative', () => {
  it('rejects a 25-point deployment, then takes a 40-point army into battle with 24 deployed and 16 in reserve', async () => {
    const h = createGameHarness()
    Object.assign(h.tables.cards[0], { cost: 3, profile: catalogue2026[0].profile })
    for (const entry of h.tables.deckCards) entry.quantity = 13
    h.tables.cards.push({ ...h.tables.cards[0], _id: 'one-point-unit', stableId: 'lanciers', name: 'Lanciers', cost: 1 })
    for (const user of [1, 2]) h.tables.deckCards.push({ _id: `lanciers-${user}`, deckId: `deck-${user}`, cardId: 'one-point-unit', quantity: 1 })
    const gameId = await h.readyFor('preparation')
    const choose = (quantity: number, cardStableId = 'archers') => h.run('updatePreparation', 1, { gameId, cardStableId, change: { quantity } })
    await choose(8)
    await choose(1, 'lanciers')
    await expect(h.run('finishPreparation', 1, { gameId })).rejects.toMatchObject(code('DEPLOYMENT_BUDGET_EXCEEDED'))
    expect((await h.run('get', 1, { gameId }))?.players[0].preparationReady).toBe(false)
    await choose(0, 'lanciers')
    ...
    for (const cell of [36, 37, 38, 39, 41, 42, 43]) await place(cell)
