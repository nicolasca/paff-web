...
## Retours de jeu — décision du 26 septembre 2026

Nicolas demande ces évolutions sur une **nouvelle branche**, puis corrige explicitement sa demande initiale de PR : conserver d’abord les changements non commités dans le dossier du projet pour la revue. Après validation locale des fonctions et des six audios, il autorise le **26 septembre** la fusion du lot dans `main` et sa publication complète en production, interface Vercel et fonctions Convex `tough-gecko-249`. Aucune PR demandée. Les profils du catalogue ne changent pas : aucun réimport `catalogue2026:apply` ni migration de decks n’est nécessaire.

- **Budgets** : deck de 40 points maximum pour jouer et déploiement de 24 points maximum. Aucun minimum de déploiement ; la réserve conserve tout le reste. Ces chiffres remplacent 33/21 ; quotas de types et capacités de cases inchangés. L’éditeur conserve la sauvegarde libre avec signalement des dépassements, la conformité étant contrôlée au choix du deck.
- **Tiens, des gobelins...** : bouton sous le plateau permettant aux Gobelins d’ajouter une nouvelle Bande sur une case libre, même absente du deck, de la réserve et de la défausse. Aucune consommation des exemplaires existants, de PS ou de stock ; chaque ajout possède une identité propre et reste manipulable comme les autres unités. Le serveur contrôle la faction, l’ordre, la place et les dépôts obsolètes. Une Bande déjà présente dans l’armée conserve son profil figé ; sinon le profil publié est copié lors du premier ajout, puis conservé.
- **R** : petite tuile et compteur = valeur actuelle, carte complète au survol/focus = valeur initiale du profil figé dans la partie. Même distinction pour les spectateurs, y compris lorsque les R actuels dépassent le profil.
- Version des nouvelles batailles : `2026-09-26-manual-1`. Les parties existantes conservent leurs cartes et ordres ; l’outil de renfort est disponible si elles possèdent l’ordre Gobelins. Le simulateur séparé n’est pas modifié par ce lot.
- **Voix des factions** : après les essais ElevenLabs, Nicolas fournit six MP3 le 26 septembre et autorise leur intégration au catalogue public, puis remplace la prise principale des Gobelins par `voice-gobs-2.mp3`. Dans la bannière de chaque faction, deux petites icônes discrètes lancent les fichiers correspondants, uniquement au clic ; aucun grand encadré ni libellé visible. Les infobulles « Écouter » et « Une autre voix » restent neutres : Nicolas veut que le ton soit une surprise, sans annoncer « sérieux », « fun » ou « décalé ». Le second bouton reprend l’icône haut-parleur déformée, avec des ondes irrégulières et une teinte discrète, à sa demande. Un seul extrait à la fois ; un second clic l’arrête et changer de faction ou quitter la page coupe le son. Les fichiers sont locaux dans `public/audio/factions/`, avec attribution `elevenlabs.io` dans le titre du lecteur. Provenance dans [audio-factions.md](audio-factions.md). La revue locale est validée et la publication en production autorisée avec le lot ; aucun abonnement payant n’est demandé.
